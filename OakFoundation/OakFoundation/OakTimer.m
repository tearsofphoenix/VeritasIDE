...
@property (nonatomic, assign) OakTimer* owner;
...
{
	//D(DBF_OakTimer, bug("%f %s (%p), %s\n", seconds, [[target description] UTF8String], target, (char const*)aSelector););
	return [self scheduledTimerWithTimeInterval:seconds target:target selector:aSelector userInfo: nil repeats:repeats];
}
