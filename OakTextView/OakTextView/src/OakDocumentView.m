...
static NSArray*  ObservedTextViewKeyPaths = nil;
...
+ (void)initialize
{
    ObservedTextViewKeyPaths =  [@[ @"selectionString", @"tabSize", @"softTabs", @"isMacroRecording"] retain];
}
...
- (id)initWithFrame:(NSRect)aRect
{
	if(self = [super initWithFrame:aRect])
	{
		...
		for(id keyPath in ObservedTextViewKeyPaths)
        {
...
- (void)updateConstraints
{
	...
	[self addConstraints: [NSLayoutConstraint constraintsWithVisualFormat: @"H:|[gutterScrollView(==gutterView)][gutterDividerView(==1)][textScrollView(>=100)]|"
                                                                  options: (NSLayoutFormatAlignAllTop
                                                                            |NSLayoutFormatAlignAllBottom)
                                                                  metrics: nil
                                                                    views: views]];
    
	[self addConstraints: [NSLayoutConstraint constraintsWithVisualFormat: @"H:|[statusBar]|"
                                                                  options: 0
                                                                  metrics: nil
                                                                    views: views]];
    
	[self addConstraints: [NSLayoutConstraint constraintsWithVisualFormat: @"V:[gutterView(==textView)]"
                                                                  options: NSLayoutFormatAlignAllTop
                                                                  metrics: nil
                                                                    views: NSDictionaryOfVariableBindings(gutterView, textView)]];
	...
	[self addConstraints: [NSLayoutConstraint constraintsWithVisualFormat: @"V:|[topView]"
                                                                  options: 0
                                                                  metrics: nil
                                                                    views: @{ @"topView" : stackedViews[0] }]];
	for(NSUInteger i = 0; i < [stackedViews count]-1; ++i)
    {
		[self addConstraint:[NSLayoutConstraint constraintWithItem: stackedViews[i]
                                                         attribute: NSLayoutAttributeBottom
                                                         relatedBy: NSLayoutRelationEqual
                                                            toItem: stackedViews[i+1]
	...
	[self addConstraints: [NSLayoutConstraint constraintsWithVisualFormat: @"V:[bottomView]|"
                                                                  options: 0
                                                                  metrics: nil
                                                                    views: @{ @"bottomView" : [stackedViews lastObject] }]];
...
- (NSImage*)gutterImage:(NSString*)aName
{
    NSImage* res = [[[NSImage imageNamed: aName
                     inSameBundleAsClass: [self class]] copy] autorelease];
	if(res)
	{
...
{
    [self setFont: [NSFont fontWithName: [textView.font fontName]
                                   size: [textView.font pointSize] + 1]];
}

- (IBAction)makeTextSmaller:(id)sender
{
    [self setFont: [NSFont fontWithName: [textView.font fontName]
                                   size: MAX([textView.font pointSize] - 1, 5)]];
}
...
//		settings_t::set(kSettingsFontSizeKey, (NSUInteger)[newFont pointSize]);
		[self setFont: newFont];
	}
}

- (void)observeValueForKeyPath: (NSString*)aKeyPath
                      ofObject: (id)observableController
                        change: (NSDictionary*)changeDictionary
                       context: (void*)userData
{
	if(observableController != textView
       || ![ObservedTextViewKeyPaths containsObject: aKeyPath])
    {
		return;
    }
	...
	{
		NSString* str = [textView valueForKey:@"selectionString"] ?: @"1";
...
//		NSUInteger i = buf.convert(sel.last().max());
...
- (void)setDocument: (OakDocument * )aDocument
{
    ///TODO
    /*
	OakDocument * oldDocument = document;
	if(oldDocument)
	{
		oldDocument->remove_callback(callback);
		oldDocument->set_visible_rect(NSStringFromRect([self.textView visibleRect]));
	}
...
- (IBAction)toggleLineNumbers:(id)sender
{
	...
	[gutterView setVisibility: isVisibleFlag
      forColumnWithIdentifier: GVLineNumbersColumnIdentifier];
	...
	}else
    {
        [[NSUserDefaults standardUserDefaults] setObject: @YES
                                                  forKey: @"DocumentView Disable Line Numbers"];
    }
}
...
- (BOOL)validateMenuItem:(NSMenuItem*)aMenuItem
{
    ...
    {
		[aMenuItem setState: ([[[textView theme] uuid] isEqual: [aMenuItem representedObject]]) ? NSOnState : NSOffState];
    }
...
{
    ///TODO
//	if(OakBundleItem * item = bundles::lookup(to_s((NSString*)[sender representedObject])))
//	{
...
{
//	NSMutableDictionary *grammars = [NSMutableDictionary dictionary];
...
//		[item setKeyEquivalentCxxString: [NSString stringWithCxxString: key_equivalent(pair->second)]];
//		[item setTarget:self];
...
//		[menu addItemWithTitle:@"No Grammars Loaded" action:@selector(nop:) keyEquivalent:@""];
	...
	[statusBar showMenu: menu
      withSelectedIndex: -1
         forCellWithTag: [sender tag]
...
//	}
	...
	[statusBar showMenu: menu
      withSelectedIndex: (index ? index-1 : 0)
         forCellWithTag: [sender tag]
                   font: [NSFont controlContentFontOfSize: [NSFont smallSystemFontSize]]
                  popup: YES];
}
...
{
//	std::multimap<NSString *, OakBundleItem *, text::less_t> ordered;
...
//		[menu addItemWithTitle:@"No Bundles Loaded" action:@selector(nop:) keyEquivalent:@""];
	...
	[statusBar showMenu: menu
      withSelectedIndex: -1
         forCellWithTag: sender ? [sender tag] : 1
                   font: [NSFont controlContentFontOfSize:[NSFont smallSystemFontSize]]
                  popup: YES];
...
- (IBAction)showTabSizeSelector:(id)sender
{
	...
	NSMenu* menu = [[NSMenu new] autorelease];
	[[menu addItemWithTitle: @"Indent Size"
                     action: @selector(dummy:)
              keyEquivalent: @""] setEnabled: NO];
	for(NSNumber *size in @[ @2, @3, @4, @8 ])
	{
		NSMenuItem* item = [menu addItemWithTitle: [NSString stringWithFormat:@"\u2003%@", size]
                                           action: @selector(takeTabSizeFrom:)
                                    keyEquivalent: @""];
		[item setTarget:self];
		[item setTag: [size integerValue]];
		if([size integerValue] == [textView tabSize])
		{
			[item setState:NSOnState];
...
//		settings_t::set(kSettingsTabSizeKey, (NSUInteger)[sender tag], document->file_type());
	}
}
...
{
//    OakBundleItem * themeItem = bundles::lookup(to_s(themeUUID));
//	if(themeItem)
//	{
...
#pragma mark - GutterView Delegate Proxy =

- (GVLineRecord )lineRecordForPosition:(CGFloat)yPos
{
    return [textView lineRecordForPosition:yPos];
}

- (GVLineRecord )lineFragmentForLine: (NSUInteger)aLine
                              column: (NSUInteger)aColumn
{
    return [textView lineFragmentForLine:aLine column:aColumn];
}
...
{
    kBookmarkNoMark,
    kBookmarkRegularMark,
    kBookmarkSearchMark
};

typedef NSInteger bookmark_state_t;

static NSString * kBookmarkType = @"bookmark";
...
//	{
//		NSString *  buf = document->buffer();
//		std::map<NSUInteger, NSString *>  marks = buf.get_marks(buf.begin(lineNumber), buf.eol(lineNumber));
//		iterate(pair, marks)
//		{
...
- (void)updateBookmarksMenu:(NSMenu*)aMenu
{
    ...
    //
//	NSString * buf = document->buffer();
//	std::map<NSUInteger, NSString *>  marks = buf.get_marks(0, buf.size(), kBookmarkType);
//	iterate(pair, marks)
//	{
...
//	{
//		NSString *& buf = document->buffer();
//		std::map<NSUInteger, NSString *>  marks = buf.get_marks(buf.begin(lineNumber), buf.eol(lineNumber), kBookmarkType);
//		iterate(pair, marks)
//		{
//			if(pair->second == kBookmarkType)
...
//	else if([columnIdentifier isEqualToString:kFoldingsColumnIdentifier])
//	{
//		[textView toggleFoldingAtLine:lineNumber recursive:OakIsAlternateKeyOrMouseEvent(NSAlternateKeyMask, [NSApp currentEvent])];
...
//
//	text::selection_t sel([textView.selectionString UTF8String]);
//	NSUInteger lineNumber = sel.last().max().line;
//
//	std::vector<NSUInteger> toRemove;
//	std::map<NSUInteger, NSString *>  marks = buf.get_marks(buf.begin(lineNumber), buf.eol(lineNumber), kBookmarkType);
//	iterate(pair, marks)
//	{
//		if(pair->second == kBookmarkType)
...
//
//	NSString *  buf = document->buffer();
//	std::pair<NSUInteger, NSString *>  pair = buf.next_mark(buf.convert(sel.last().max()), kBookmarkType);
...
//
//	NSString *  buf = document->buffer();
//	std::pair<NSUInteger, NSString *>  pair = buf.prev_mark(buf.convert(sel.last().max()), kBookmarkType);
//	if(pair.second != NULL_STR)
//		textView.selectionString = [NSString stringWithCxxString:buf.convert(pair.first)];
}
