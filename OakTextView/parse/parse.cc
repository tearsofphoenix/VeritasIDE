...
namespace parse
{
	std::vector< std::pair<scope::selector_t, rule_ptr> >& injected_grammars ()
	{
		...
		if(needs_setup)
		{
			...
			{
				NSString * injectionSelector = (*item)->value_for_field(bundles::kFieldGrammarInjectionSelector);
	...
	static scope::scope_t create_scope (scope::scope_t  current_scope, NSString * format_string, regexp::match_t  match)
	{
	...
	static void apply_captures (scope::scope_t scope, regexp::match_t  m, repository_ptr  captures, std::map<NSUInteger, scope::scope_t>& res, bool firstLine)
	{
		...
		std::multimap<std::pair<NSUInteger, ssize_t>, rule_ptr> rules;

		repository_t::const_iterator ruleIter = captures->begin();
		std::multimap<NSString *, std::pair<NSUInteger, NSUInteger> >::const_iterator indexIter = m.capture_indices().begin();
		...
		std::vector< std::pair<NSUInteger, scope::scope_t> > stack;
		iterate(it, rules)
		{
			NSUInteger from = it->first.first;
			for(; !stack.empty() && stack.back().first <= from; stack.pop_back())
				scope = res[stack.back().first] = stack.back().second;

			NSUInteger to = it->first.first - it->first.second;
	...
	static NSUInteger collect_rule (rule_ptr  base, char const* first, char const* last, NSUInteger anchor, NSUInteger i, bool firstLine, rule_ptr rule, std::set<ranked_match_t>& res, std::map<NSUInteger, regexp::match_t>& match_cache, std::set<NSUInteger>& unique, NSUInteger rank)
	{
		...
		{
			NSString * name = rule->include_string;
		...
		if(rule->match_pattern)
		{
			...
			std::map<NSUInteger, regexp::match_t>::iterator it = match_cache.find(rule->rule_id);
			...
			{
				regexp::match_t  match = regexp::search(fix_anchor(rule->match_pattern, anchor, i, firstLine), first, last, first + i);
	...
	static void collect_rules (rule_ptr  base, char const* first, char const* last, NSUInteger i, bool firstLine, stack_ptr  stack, std::set<ranked_match_t>& res, std::map<NSUInteger, regexp::match_t>& match_cache)
	{
		...
		if(stack->end_pattern)
		{
			...
			if(regexp::match_t  match = regexp::search(fix_anchor(stack->end_pattern, stack->anchor, i, firstLine), first, last, first + i))
				res.insert((ranked_match_t){ stack->rule, match, stack->apply_end_last ? SIZE_T_MAX : 0 });
		}

		std::set<NSUInteger> unique;
		NSUInteger rank = collect_children(base, first, last, stack->anchor, i, firstLine, stack->rule->children, res, match_cache, unique);
	...
	stack_ptr parse (char const* first, char const* last, stack_ptr stack, std::map<NSUInteger, scope::scope_t>& scopes, bool firstLine, NSUInteger i)
	{
		...
		{
			if(regexp::match_t  m = regexp::search((*it)->while_pattern, first, last, first + i))
			{
				D(DBF_Parser_Flow, bug("while match %zu-%zu\n", m.begin(), m.end()););

				rule_ptr  rule = (*it)->rule;
				if(rule->scope_string != NULL_STR)
		...
		std::map<NSUInteger, regexp::match_t> match_cache;
		...
		while(!rules.empty())
		{
			...
			if(m.match.begin() < i)
			{
				regexp::pattern_t  ptrn = m.rank == 0 || m.rank == SIZE_T_MAX ? stack->end_pattern : m.rule->match_pattern;
			...
			else // regular match-rule
			{
				...
				if(rule->scope_string != NULL_STR)
				{
					scope::scope_t  newScope = create_scope(scope, rule->scope_string, m.match);
