...
static NSUInteger count_columns (NSString *  buf, NSUInteger from, NSUInteger to)
{
	NSUInteger const tabSize = buf.indent().tab_size();
	NSUInteger col = 0;
	NSString * str = buf.substr(from, to);
...
namespace ng
{
	...
	layout_t::layout_t (NSString *& buffer, theme_ptr  theme, NSString * fontName, CGFloat fontSize, bool softWrap, NSUInteger wrapColumn, NSString * folded, ng::layout_t::margin_t  margin) : _folds(new folds_t(buffer)), _buffer(buffer), _theme(theme), _font_name(fontName), _font_size(fontSize), _tab_size(buffer.indent().tab_size()), _wrapping(softWrap), _wrap_column(wrapColumn), _margin(margin)
	{
		struct parser_callback_t : ng::callback_t
		{
			...
			void did_replace (NSUInteger from, NSUInteger to, NSString * str) { layout.did_erase(from, to); layout.did_insert(from, from + str.size()); }
	...
	void layout_t::set_font (NSString * fontName, CGFloat fontSize)
	{
	...
	void layout_t::set_tab_size (NSUInteger tabSize)
	{
	...
	void layout_t::set_margin (margin_t  margin)
	{
		_dirty_rects.push_back(OakRectMake(0, 0, width(), height()));
		_margin = margin;
		_dirty_rects.push_back(OakRectMake(0, 0, width(), height()));
	...
	void layout_t::set_viewport_size (CGSize size)
	{
		...
		NSUInteger oldWrapColumn = effective_wrap_column();
		_viewport_size = size;
	...
	void layout_t::did_fold (NSUInteger from, NSUInteger to)
	{
		did_erase(from, to);
		did_insert(from, to);
	}
	...
	layout_t::row_tree_t::iterator layout_t::row_for_offset (NSUInteger i) const
	{
	...
	CGRect layout_t::full_width (CGRect  rect) const
	{
		return OakRectMake(0, CGRectGetMinY(rect), width(), CGRectGetHeight(rect));
	}

	CGRect layout_t::full_height (CGRect  rect) const
	{
	...
	bool layout_t::effective_soft_wrap (row_tree_t::iterator rowIter) const
	{
		...
		id softWrapValue = bundles::value_for_setting("softWrap", scope, &softWrapItem);
	...
	CGRect layout_t::rect_for_range (NSUInteger first, NSUInteger last) const
	{
	...
	std::vector<CGRect> layout_t::rects_for_ranges (OakSelectionRanges *  ranges, kRectsIncludeMode mode) const
	{
		...
		citerate(range, ng::dissect_columnar(_buffer, ranges))
		{
			...
			else
			{
				...
				if(CGRectGetMinY(r1) == CGRectGetMinY(r2) && CGRectGetHeight(r1) == CGRectGetHeight(r2))
				{
					res.push_back(OakRectMake(MIN(r1.origin.x, r2.origin.x), r1.origin.y, fabs(r2.origin.x - r1.origin.x), r1.size.height));
				}
				else
				{
	...
	void layout_t::did_erase (NSUInteger from, NSUInteger to)
	{
		ASSERT_LE(from, to);
		if(from == to)
			return;
		...
		{
			NSUInteger base = fromRow->offset._length;
			NSUInteger prefixLenToErase  = to   - toRow->offset._length;
			NSUInteger prefixLenToInsert = from - fromRow->offset._length;
	...
	void layout_t::did_insert (NSUInteger first, NSUInteger last)
	{
		ASSERT_LE(first, last);
		if(first == last)
			return;
		...
		for(NSUInteger pos = first; pos != last; )
		{
			NSUInteger eol = MIN(_buffer.eol(_buffer.convert(pos).line), last);
		...
		std::vector< std::pair<NSUInteger, NSUInteger> > foldedRanges;
		ssize_t nestCount = 0;
	...
	void layout_t::begin_refresh_cycle (OakSelectionRanges *  selection, OakSelectionRanges *  highlightRanges)
	{
	...
	static std::pair<NSUInteger, NSUInteger> misspelled_range_at (NSString *  buffer, NSUInteger index)
	{
		NSUInteger line = buffer.convert(index).line;
		NSUInteger from = buffer.begin(line);
		NSUInteger to   = buffer.eol(line);
		...
		if(it != map.end())
		{
			NSUInteger first = from + it->first;
			NSUInteger last  = ++it != map.end() ? from + it->first : to;
			if(OakCap(first, index, last) == index)
				return std::make_pair(first, last);
		}
		return std::make_pair(0, 0);
	}

	std::vector<CGRect> layout_t::end_refresh_cycle (OakSelectionRanges *  selection, CGRect visibleRect, OakSelectionRanges *  highlightRanges)
	{
		std::vector<CGRect> res;
		if(_refresh_counter == 1)
		{
			...
			{
				std::pair<NSUInteger, NSUInteger> ranges[] = { misspelled_range_at(_buffer, _pre_refresh_caret), misspelled_range_at(_buffer, selection.last().last.index) };
	...
	void layout_t::did_update_scopes (NSUInteger from, NSUInteger to)
	{
	...
	static index_t advance (buffer_t  buffer, NSUInteger caret, NSUInteger column, NSUInteger eol)
	{
		NSUInteger const tabSize = buffer.indent().tab_size();
		NSUInteger len = 0;
		NSString * str = buffer.substr(caret, eol);
		citerate(ch, diacritics::make_range(str.data(), str.data() + str.size()))
		{
			if(len == column)
	...
	void layout_t::draw (ng::context_t  context, CGRect visibleRect, bool isFlipped, bool showInvisibles, OakSelectionRanges *  selection, OakSelectionRanges *  highlightRanges, bool drawBackground, CGColorRef textColor)
	{
		...
		if(drawBackground)
			OakRenderFillRect(context, background, visibleRect);
		...
		if(_draw_wrap_column)
			OakRenderFillRect(context, baseColors.margin_indicator, OakRectMake(_margin.left + _metrics->column_width() * effective_wrap_column(), CGRectGetMinY(visibleRect), 1, CGRectGetHeight(visibleRect)));
		...
		citerate(range, selection)
		{
			citerate(rect, rects_for_ranges(*range, kRectsIncludeSelections))
			{
				...
				OakRenderFillRect(context, selColor, *rect);
				if(!_is_key)
					CFRelease(selColor);
			}
		}
		...
		{
			CGRect const r = rect_for_range(range->min().index, range->max().index);
			OakRenderFillRect(context, baseColors.marked_text_border, CGRectInset(r, -2, -2));
			OakRenderFillRect(context, baseColors.marked_text_background, CGRectInset(r, -1, -1));
		}
		...
		if(_draw_caret && !_drop_marker)
		{
			citerate(range, selection)
			{
				citerate(rect, rects_for_ranges(*range, kRectsIncludeCarets))
					OakRenderFillRect(context, _theme->styles_for_scope(_buffer.scope(range->min().index).right, _font_name, _font_size).caret(), *rect);
			}
		}

		if(_drop_marker)
			OakRenderFillRect(context, baseColors.drop_marker, rect_at_index(_drop_marker));
	}
	...
	void layout_t::remove_enclosing_folds (NSUInteger from, NSUInteger to)
	{
	...
	void layout_t::toggle_all_folds_at_level (NSUInteger level)
	{
	...
	NSString * layout_t::row_to_s (row_tree_t::value_type  info)
	{
