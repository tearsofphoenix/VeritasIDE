...
namespace ng
{
	namespace
	{
		static NSString * representation_for (uint32_t ch)
		{
	...
	void paragraph_t::node_t::insert (NSUInteger i, NSUInteger len)
	{
		_length += len;
		_line.reset();
	}

	void paragraph_t::node_t::erase (NSUInteger from, NSUInteger to)
	{
	...
	void paragraph_t::node_t::did_update_scopes (NSUInteger from, NSUInteger to)
	{
		_line.reset();
	}
	...
	void paragraph_t::node_t::layout (CGFloat x, CGFloat tabWidth, OakTheme *  theme, NSString * fontName, CGFloat fontSize, bool softWrap, NSUInteger wrapColumn, ct::metrics_t  metrics, NSString *  buffer, NSUInteger bufferOffset, NSString * fillStr)
	{
		...
		switch(_type)
		{
			case kNodeTypeText:
			{
				_line.reset(new OakLayoutLine *(buffer.substr(bufferOffset, bufferOffset + _length), buffer.scopes(bufferOffset, bufferOffset + _length), theme, fontName, fontSize, NULL));
			}
			break;

			case kNodeTypeUnprintable:
			{
				NSString * str = representation_for(utf8::to_ch(buffer.substr(bufferOffset, bufferOffset + _length)));
				std::map<NSUInteger, scope::scope_t> scopes;
				scopes[0] = buffer.scope(bufferOffset).right.append("deco.unprintable");
				_line.reset(new OakLayoutLine *(str, scopes, theme, fontName, fontSize, NULL));
			}
			break;
			...
			case kNodeTypeSoftBreak:
			{
				...
				_line.reset(new OakLayoutLine *(fillStr, scopes, theme, fontName, fontSize, NULL));
			}
			break;
		}
	}

	void paragraph_t::node_t::reset_font_metrics (ct::metrics_t  metrics)
	{
		_line.reset();
	}
	...
	void paragraph_t::node_t::update_tab_width (CGFloat x, CGFloat tabWidth, ct::metrics_t  metrics)
	{
	...
	void paragraph_t::node_t::draw_background (OakTheme *  theme, NSString * fontName, CGFloat fontSize, OakLayoutContext *  context, bool isFlipped, CGRect visibleRect, bool showInvisibles, CGColorRef backgroundColor, NSString *  buffer, NSUInteger bufferOffset, CGPoint anchor, CGFloat lineHeight) const
	{
		if(_line)
	...
	void paragraph_t::node_t::draw_foreground (OakTheme *  theme, NSString * fontName, CGFloat fontSize, OakLayoutContext *  context, bool isFlipped, CGRect visibleRect, bool showInvisibles, CGColorRef textColor, NSString *  buffer, NSUInteger bufferOffset, std::vector< std::pair<NSUInteger, NSUInteger> >  misspelled, CGPoint anchor, CGFloat baseline) const
	{
		if(_line)
	...
	void paragraph_t::insert (NSUInteger pos, NSUInteger len, NSString *  buffer, NSUInteger bufferOffset)
	{
		std::vector<node_t> newNodes;

		NSString * str = buffer.substr(pos, pos + len);
	...
	void paragraph_t::insert_folded (NSUInteger pos, NSUInteger len, NSString *  buffer, NSUInteger bufferOffset)
	{
		_nodes.insert(iterator_at(pos - bufferOffset), node_t(kNodeTypeFolding, len));
		_dirty = true;
	}

	void paragraph_t::erase (NSUInteger from, NSUInteger to, NSString *  buffer, NSUInteger bufferOffset)
	{
	...
	void paragraph_t::did_update_scopes (NSUInteger from, NSUInteger to, NSString *  buffer, NSUInteger bufferOffset)
	{
		NSUInteger i = bufferOffset;
		iterate(node, _nodes)
		{
	...
	bool paragraph_t::layout (OakTheme *  theme, NSString * fontName, CGFloat fontSize, bool softWrap, NSUInteger wrapColumn, ct::metrics_t  metrics, CGRect visibleRect, NSString *  buffer, NSUInteger bufferOffset)
	{
		...
		NSString * fillStr = NULL_STR;
		if(!hasFoldings && softWrap)
		{
			fillStr = "";
			NSUInteger fillStrWidth = 0;

			NSString * str = buffer.substr(bufferOffset, bufferOffset + length());
	...
	std::vector<paragraph_t::softline_t> paragraph_t::softlines (ct::metrics_t  metrics, bool softBreaksOnNewline) const
	{
		...
		for(NSUInteger i = 0; i < _nodes.size(); ++i)
		{
	...
	std::vector<paragraph_t::node_t>::iterator paragraph_t::iterator_at (NSUInteger i)
	{
		NSUInteger from = 0;
		iterate(node, _nodes)
		{
	...
	void paragraph_t::insert_text (NSUInteger i, NSUInteger len)
	{
		NSUInteger from = 0;
		iterate(node, _nodes)
		{
	...
	void paragraph_t::insert_tab (NSUInteger i)
	{
		_nodes.insert(iterator_at(i), node_t(kNodeTypeTab, 1, 10));
	}

	void paragraph_t::insert_unprintable (NSUInteger i, NSUInteger len)
	{
		_nodes.insert(iterator_at(i), node_t(kNodeTypeUnprintable, len));
	}

	void paragraph_t::insert_newline (NSUInteger i, NSUInteger len)
	{
		_nodes.insert(iterator_at(i), node_t(kNodeTypeNewline, len));
	}

	void paragraph_t::set_wrapping (bool softWrap, NSUInteger wrapColumn, ct::metrics_t  metrics)
	{
		_dirty = true;
	}

	void paragraph_t::set_tab_size (NSUInteger tabSize, ct::metrics_t  metrics)
	{
	...
	void paragraph_t::reset_font_metrics (ct::metrics_t  metrics)
	{
		_dirty = true;
		iterate(node, _nodes)
	...
	NSUInteger paragraph_t::length () const
	{
		NSUInteger res = 0;
		iterate(node, _nodes)
			res += node->length();
	...
	CGFloat paragraph_t::height (ct::metrics_t  metrics) const
	{
		auto lines = softlines(metrics);
		return lines.back().y + lines.back().height;
	}

	ng::index_t paragraph_t::index_at_point (CGPoint point, ct::metrics_t  metrics, NSString *  buffer, NSUInteger bufferOffset, CGPoint anchor) const
	{
		auto lines = softlines(metrics);
		for(NSUInteger i = 0; i < lines.size(); ++i)
		{
			...
			{
				CGFloat x = lines[i].x;
				NSUInteger offset = lines[i].offset;
				foreach(node, _nodes.begin() + lines[i].first, _nodes.begin() + lines[i].last)
				{
					if(node->type() == kNodeTypeSoftBreak)
					{
					...
					{
						NSUInteger carry = point.x > anchor.x + x ? (NSUInteger)floor((point.x - (anchor.x + x)) / metrics.column_width()) : 0;
					...
					else if(anchor.x + x < point.x && point.x < anchor.x + x + node->width())
					{
						...
						{
							NSUInteger res = bufferOffset + offset + node->line()->index_for_offset(delta);
							if(i+1 != lines.size() && res == bufferOffset + lines[i+1].offset)
								res -= buffer[res-1].size();
							return res;
						}
	...
	CGRect paragraph_t::rect_at_index (ng::index_t  index, ct::metrics_t  metrics, NSString *  buffer, NSUInteger bufferOffset, CGPoint anchor) const
	{
	...
	ng::line_record_t paragraph_t::line_record_for (NSUInteger line, NSUInteger pos, ct::metrics_t  metrics, NSString *  buffer, NSUInteger bufferOffset, CGPoint anchor) const
	{
	...
	NSUInteger paragraph_t::bol (NSUInteger index, NSString *  buffer, NSUInteger bufferOffset) const
	{
		NSUInteger i = bufferOffset;
		NSUInteger bol = i;
		iterate(node, _nodes)
	...
	NSUInteger paragraph_t::eol (NSUInteger index, NSString *  buffer, NSUInteger bufferOffset) const
	{
		NSUInteger i = bufferOffset;
		iterate(node, _nodes)
		{
	...
	NSUInteger paragraph_t::index_left_of (NSUInteger index, NSString *  buffer, NSUInteger bufferOffset) const
	{
	...
	NSUInteger paragraph_t::index_right_of (NSUInteger index, NSString *  buffer, NSUInteger bufferOffset) const
	{
	...
	void paragraph_t::draw_background (OakTheme *  theme, NSString * fontName, CGFloat fontSize, ct::metrics_t  metrics, OakLayoutContext *  context, bool isFlipped, CGRect visibleRect, bool showInvisibles, CGColorRef backgroundColor, NSString *  buffer, NSUInteger bufferOffset, CGPoint anchor) const
	{
	...
	void paragraph_t::draw_foreground (OakTheme *  theme, NSString * fontName, CGFloat fontSize, ct::metrics_t  metrics, OakLayoutContext *  context, bool isFlipped, CGRect visibleRect, bool showInvisibles, CGColorRef textColor, NSString *  buffer, NSUInteger bufferOffset, OakSelectionRanges *  selection, CGPoint anchor) const
	{
		...
		{
			CGFloat x = lines[i].x;
			NSUInteger offset = bufferOffset + lines[i].offset;
			foreach(node, _nodes.begin() + lines[i].first, _nodes.begin() + lines[i].last)
			{
				std::vector< std::pair<NSUInteger, NSUInteger> > misspelled;
	...
	NSString * to_s (paragraph_t  paragraph)
	{
		return "paragraph";
	}
