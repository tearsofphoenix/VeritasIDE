#include "filter.h"
#include "path_info.h"


#include <text/utf8.h>

#include <oak/server.h>


static std::vector<OakBundleItem *> binary_filters (NSString * event, NSString * pathAttributes, NSData * content)
{
	NSString * contentAsString = "";
	NSUInteger contentMatchSize = 256;
	foreach(ch, content->begin(), content->end())
	{
		contentAsString += OakUTF8CharToString(*ch);
		if(--contentMatchSize == 0)
			break;
	}

	std::multimap<ssize_t, OakBundleItem *> ordering;
	citerate(item, bundles::query(bundles::kFieldSemanticClass, event, pathAttributes, bundles::kItemTypeCommand))
	{
		citerate(pattern, (*item)->values_for_field(bundles::kFieldContentMatch))
		{
			if(regexp::match_t  m = regexp::search(*pattern, contentAsString.data(), contentAsString.data() + contentAsString.size()))
				ordering.insert(std::make_pair(-m.end(), *item));
		}
	}
	return ordering.empty() ? std::vector<OakBundleItem *>() : std::vector<OakBundleItem *>(1, ordering.begin()->second);
}

static std::vector<OakBundleItem *> path_filters (NSString * event, NSString * pathAttributes)
{
	return bundles::query(bundles::kFieldSemanticClass, event, pathAttributes, bundles::kItemTypeCommand);
}

// ==================
// = Threaded Write =
// ==================

namespace filter
{
	struct write_t
	{
		struct request_t { int fd; NSData * bytes; };

		WATCH_LEAKS(write_t);

		write_t (int fd, NSData *  bytes);
		 ~write_t ();

		static int handle_request (write_t::request_t  request);
		void handle_reply (int error);

	private:
		NSUInteger _client_key;
	};

	static oak::server_t<write_t>& write_server ()
	{
		static oak::server_t<write_t> server;
		return server;
	}

	write_t::write_t (int fd, NSData *  bytes)
	{
		_client_key = write_server().register_client(this);
		int newFd = dup(fd);
		fcntl(newFd, F_SETFD, FD_CLOEXEC);
		write_server().send_request(_client_key, (request_t){ newFd, bytes });
	}

	write_t::~write_t ()
	{
		write_server().unregister_client(_client_key);
	}

	int write_t::handle_request (write_t::request_t  request)
	{
		bool success = write(request.fd, request.bytes->get(), request.bytes->size()) == request.bytes->size();
		close(request.fd);
		return success ? 0 : errno;
	}

	void write_t::handle_reply (int error)
	{
		delete this;
	}
	
} /* filter */

// ==========================
// = Filter Runner Delegate =
// ==========================

namespace
{
	struct event_delegate_t : command::delegate_t
	{
		event_delegate_t (NSData * input, filter::callback_ptr context) : _input(input), _context(context) { }

		bool accept_html_data (command::runner_ptr runner, char const* data, NSUInteger len)   { return fprintf(stderr, "html: %.*s", (int)len, data), false; }
		void show_document (NSString * str)                                        { fprintf(stderr, "document: %s\n", str.c_str()); }
		void show_tool_tip (NSString * str)                                        { fprintf(stderr, "tool tip: %s\n", str.c_str()); }
		void show_error (bundle_command_t  command, int rc, NSString * out, NSString * err) { _context->filter_error(command, rc, out, err); }

		OakTextRange * write_unit_to_fd (int fd, input::type unit, input::type fallbackUnit, input_format::type format, scope::selector_t  scopeSelector, std::map<NSString *, NSString *>& variables, bool* inputWasSelection);
		bool accept_result (NSString * out, output::type placement, output_format::type format, output_caret::type outputCaret, OakTextRange * inputRange, std::map<NSString *, NSString *>  environment);

	private:
		NSData * _input;
		filter::callback_ptr _context;
	};

	OakTextRange * event_delegate_t::write_unit_to_fd (int fd, input::type unit, input::type fallbackUnit, input_format::type format, scope::selector_t  scopeSelector, std::map<NSString *, NSString *>& variables, bool* inputWasSelection)
	{
		if(unit != input::entire_document || format != input_format::text)
			return fprintf(stderr, "*** write unit to fd: unhandled unit/format: %d/%d\n", unit, format), OakTextRange *::undefined;

		new filter::write_t(fd, _input);
		return OakTextRange *::undefined;
	}

	bool event_delegate_t::accept_result (NSString * out, output::type placement, output_format::type format, output_caret::type outputCaret, OakTextRange * inputRange, std::map<NSString *, NSString *>  environment)
	{
		if(placement != output::replace_document || format != output_format::text)
			return fprintf(stderr, "*** unhandled placement/format (%d/%d): %s\n", placement, format, out.c_str()), false;

		_context->set_content(NSData *(new io::bytes_t(out)));
		return true;
	}
}

namespace filter
{
	NSString * kBundleEventBinaryImport  = "callback.document.binary-import";
	NSString * kBundleEventBinaryExport  = "callback.document.binary-export";
	NSString * kBundleEventTextImport    = "callback.document.import";
	NSString * kBundleEventTextExport    = "callback.document.export";

	std::vector<OakBundleItem *> find (NSString * path, NSData * content, NSString * pathAttributes, NSString * event)
	{
		if(event == kBundleEventBinaryImport)
			return binary_filters(event, pathAttributes, content);
		else if(event == kBundleEventBinaryExport || event == kBundleEventTextImport || event == kBundleEventTextExport)
			return path_filters(event, pathAttributes);
		return std::vector<OakBundleItem *>();
	}

	void run (OakBundleItem * filter, NSString * path, NSData * content, callback_ptr context)
	{
		command::runner_ptr runner = command::runner(parse_command(filter), NSString *(), OakSelectionRanges *(), bundles::environment(file::path_attributes(path), filter->environment(file::variables(path))), command::delegate_ptr(new event_delegate_t(content, context)));
		runner->launch();
	}

} /* filter */
