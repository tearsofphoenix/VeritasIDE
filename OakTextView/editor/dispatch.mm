...
namespace ng
{
	action_t to_action (NSString * sel)
	{
		static struct { NSString * selector; action_t action; } const actions[] =
		{
		...
		for(NSUInteger i = 0; i < sizeofA(actions); ++i)
		{
	...
	void editor_t::find_dispatch (NSMutableDictionary *  args)
	{
		...
		NSString * action = "findNext", searchFor = NULL_STR, replaceWith = NULL_STR;
	...
	void editor_t::snippet_dispatch (NSMutableDictionary *  plist, std::map<NSString *, NSString *>  variables)
	{
		NSString * str;
	...
	void editor_t::execute_dispatch (NSMutableDictionary *  plist, std::map<NSString *, NSString *>  variables)
	{
		document::run(parse_command(convert_command_from_v1(plist)), _buffer, _selections, _document, variables);
	}

	void editor_t::macro_dispatch (NSMutableDictionary *  plist, std::map<NSString *, NSString *>  variables)
	{
		...
		{
			NSMutableDictionary * dict = boost::get<NSMutableDictionary *>(*command); // TODO assert this!
			NSString * sel, str;
