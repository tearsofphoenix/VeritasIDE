...
namespace ng
{
	static std::map<NSUUID *, editor_ptr>& editors ()
	{
		static std::map<NSUUID *, editor_ptr> editors;
		return editors;
	}

	editor_ptr editor_for_document (document::OakDocument * document)
	{
		static struct document_close_callback_t : document::document_t::callback_t
		{
			...
			void handle_document_event (document::OakDocument * document, event_t event)
			{
		...
		std::map<NSUUID *, editor_ptr>::iterator editor = editors().find(document->identifier());
	...
	static NSStringCompareOptions convert (std::map<NSString *, NSString *>  options)
	{
		static struct { NSString * key; NSStringCompareOptions flag; } const map[] =
		{
		...
		NSStringCompareOptions res = ;
		for(NSUInteger i = 0; i < sizeofA(map); ++i)
		{
			std::map<NSString *, NSString *>::const_iterator it = options.find(map[i].key);
	...
	_OutputIter transpose_selections (buffer_t  _buffer, ranges_t  _selections, _OutputIter out)
	{
		ranges_t sel;
	...
	static NSUInteger const kColumnar = 1 << 0;
	static NSUInteger const kReversed = 1 << 1;

	struct preserve_selection_helper_t : callback_t
	{
		preserve_selection_helper_t (buffer_t& buffer, ranges_t  marks) : _buffer(buffer)
		{
			iterate(range, marks)
			{
				if(range->empty())
				...
				{
					NSUInteger userInfo = (range->columnar ? kColumnar : 0) | (range->last < range->first ? kReversed : 0);
		...
		void will_replace (NSUInteger from, NSUInteger to, NSString * str)
		{
			iterate(mark, _marks)
			{
				NSUInteger& index = mark->position.index;
				if(OakCap(from, index, to) == index)
				{
					if(mark->type == mark_t::kUnpairedMark || (index != from && index != to))
					{
						index = from + str.size() - MIN(to - index, str.size());
					}
					else
					{
						index = from;
	...
	private:
		struct mark_t
		{
			...
			NSUInteger user_info;

			mark_t (index_t  position, mark_type type, NSUInteger user_info = 0) : position(position), type(type), user_info(user_info) { }
		};
	...
	static ranges_t replace_helper (NSString *& buffer, snippet_controller_t& snippets, std::multimap<range_t, NSString *>  replacements)
	{
		ranges_t res;
		...
		iterate(p1, replacements)
		{
			...
			NSString * pad = orgRange.freehanded && orgRange.first.carry ? NSString *(orgRange.first.carry, ' ') : "";
			orgRange.first.carry = orgRange.last.carry = 0;

			std::vector< std::pair<range_t, NSString *> >  real = snippets.replace(orgRange.first.index, orgRange.last.index, pad + p1->second);
			iterate(p2, real)
			{
				range_t  range   = p2->first;
				NSString * str = p2->second;

				NSUInteger from = range.first.index + adjustment, to = range.last.index + adjustment;
				NSUInteger caret = buffer.replace(from, to, str);
	...
	static NSString * dummy;

	void editor_t::setup ()
	{
		set_clipboard(OakCreateSimpleClipboard());
		set_find_clipboard(OakCreateSimpleClipboard());
		set_replace_clipboard(OakCreateSimpleClipboard());
		set_yank_clipboard(OakCreateSimpleClipboard());
	}
	...
	editor_t::editor_t (document::OakDocument * document) : _buffer(document->buffer()), _document(document)
	{
	...
	struct my_clipboard_entry_t : clipboard_t::entry_t
	{
		...
		std::map<NSString *, NSString *>  options () const { return _options; }
	private:
	...
	clipboard_t::entry_ptr editor_t::copy (NSString *  buffer, OakSelectionRanges *  selections)
	{
		...
		if(selections.size() == 1)
		{
			...
			if(from.line != to.line)
			{
				if(from.column != 0)
				{
					NSString * leading = buffer.substr(buffer.begin(from.line), sel.min().index);
	...
	ranges_t editor_t::snippet (NSUInteger from, NSUInteger to, NSString * str, std::map<NSString *, NSString *>  variables)
	{
		
		NSString * indent = _buffer.substr(_buffer.begin(_buffer.convert(from).line), from);
		NSUInteger i = 0;
		while(i < indent.size() && text::is_space(indent[i]))
			++i;
		indent.resize(i);

		snippet::snippet_t  snippet = snippet::parse(str, variables, indent, _buffer.indent(), &callback);
	...
	std::vector<NSString *>  editor_t::choices () const
	{
		return _snippets.choices();
	}

	NSString * editor_t::placeholder_content (ng::range_t* placeholderSelection) const
	{
		if(_snippets.empty())
			return NULL_STR;
	...
	void editor_t::set_placeholder_content (NSString * str, NSUInteger selectFrom)
	{
		std::multimap<range_t, NSString *> map;
	...
	OakSelectionRanges * editor_t::paste (NSString *& buffer, OakSelectionRanges *  selections, snippet_controller_t& snippets, clipboard_t::entry_ptr entry)
	{
		...
		NSString * str                            = entry->content();
		std::map<NSString *, NSString *> options = entry->options();
		...
		{
			std::vector<NSString *> words = text::split(str, "\n");
		...
		if(fragments > 1 && selections.size() == 1)
		{
			...
			if(columnar)
			{
				...
				NSUInteger n      = buffer.convert(caret.index).line;
				NSUInteger col    = visual_distance(buffer, buffer.begin(n), caret);
				...
				citerate(line, text::tokenize(str.begin(), str.end(), '\n'))
				{
					...
					else if(n < buffer.lines())
					{
						...
						insertions.insert(std::make_pair(pos.index, NSString *(pos.carry, ' ') + *line));
					}
					else
					{
		...
		{
			NSUInteger const index = selections.last().min().index;
			NSUInteger const line  = buffer.convert(index).line;
			NSString * leftOfCaret = buffer.substr(buffer.begin(line), index);

			if(text::is_blank(leftOfCaret.data(), leftOfCaret.data() + leftOfCaret.size()))
			{
				NSUInteger const tabSize    = buffer.indent().tab_size();
				NSUInteger const indentSize = buffer.indent().indent_size();

				if(indent != NULL_STR)
					str = indent + str;
				if(complete)
				{
					NSString * rightOfCaret = buffer.substr(index, buffer.eol(line));
				...
				std::vector< std::pair<char const*, char const*> >  v = text::to_lines(str.data(), str.data() + str.size());
				iterate(it, v)
				{
					if(!text::is_blank(it->first, it->second))
						minIndent = MIN(indent::leading_whitespace(it->first, it->second, tabSize), minIndent);
				}

				id pasteBehaviorValue = bundles::value_for_setting("indentOnPaste", buffer.scope(index));
				NSString * pasteBehavior = boost::get<NSString *>(&pasteBehaviorValue);
				...
				else if(!pasteBehavior || *pasteBehavior != "disable")
				{
					...
					iterate(it, v)
					{
						if(fsm.is_ignored(NSString *(it->first, it->second)))
							continue;
	...
	struct indent_helper_t : ng::callback_t
	{
		...
		~indent_helper_t ()
		{
			...
			iterate(pair, _lines)
			{
				NSUInteger n = pair->first;
				NSUInteger bol = _buffer.begin(n);
				NSUInteger eos = bol;

				NSString * line = _buffer.substr(bol, _buffer.eol(n));
				int actual = indent::leading_whitespace(line.data(), line.data() + line.size(), _buffer.indent().tab_size());
	...
	static NSString * find_paired (NSString * str, OakScopeContext *  scope)
	{
		id typingPairs = bundles::value_for_setting("smartTypingPairs", scope);
		if(plist::array_t const* typingPairsArray = boost::get<plist::array_t>(&typingPairs))
		{
			iterate(pair, *typingPairsArray)
			{
				if(plist::array_t const* pairArray = boost::get<plist::array_t>(&*pair))
				{
					if(pairArray->size() == 2)
					{
						NSString * left  = boost::get<NSString *>(&(*pairArray)[0]);
						NSString * right = boost::get<NSString *>(&(*pairArray)[1]);
	...
	void editor_t::insert_with_pairing (NSString * str, BOOL indentCorrections, NSString * scopeAttributes)
	{
		if(!has_selection())
		{
			NSUInteger const caret = _selections.last().last.index;
		...
		NSString * autoInsert = find_paired(str, scope(scopeAttributes));
		...
		else
		{
			...
			{
				NSUInteger const caret = _selections.last().last.index;
				NSString * leftOfCaret = _buffer.substr(_buffer.begin(_buffer.convert(caret).line), caret);
	...
	void editor_t::snippet (NSString * str, std::map<NSString *, NSString *>  variables)
	{
		NSUInteger from = _selections.last().min().index;
		NSUInteger to   = _selections.last().max().index;
	...
	void editor_t::perform (action_t action, layout_t const* layout, BOOL indentCorrections, NSString * scopeAttributes)
	{
		...
		{
			NSUInteger caret = _selections.last().last.index;
			NSUInteger from  = caret - (0 < caret ? _buffer[caret-1].size() : 0);
			if(caret && _buffer.pairs().is_first(from))
			{
				NSUInteger other = _buffer.pairs().counterpart(from);
				NSUInteger to    = other + _buffer[other].size();
				if(other == caret)
				{
		...
		switch(action)
		{
			...
			case kFindAllInSelection:
			{
				...
				{
					NSStringCompareOptions options = convert(findEntry->options());
			...
			case kDeleteToMark:
			{
				std::map<NSUInteger, NSString *>  marks = _buffer.get_marks(0, _buffer.size(), kSingleMarkType);
				if(marks.size() == 1)
				{
					std::multimap<range_t, NSString *> replacements;
			...
			case kTranspose:
			{
				...
				if(_selections.size() > 1 && not_empty(_buffer, _selections))
				{
					...
					std::vector<NSString *> strings;
					std::transform(ranges.begin(), ranges.end(), back_inserter(strings), [this](range_t  r){ return _buffer.substr(r.min().index, r.max().index); });
					std::next_permutation(strings.begin(), strings.end());
					std::transform(ranges.begin(), ranges.end(), strings.begin(), inserter, [](range_t  r, NSString * str){ return std::make_pair(r, str); });
				}
				else
				{
			...
			case kIndent:
			{
				...
				NSUInteger to   = p1.line + (p1.column != 0 || p0.line == p1.line ? 1 : 0);
			...
			{
				NSUInteger wrapColumn = layout ? layout->effective_wrap_column() : 80;
				...
				if(_selections.last().columnar)
				{
					...
					NSString * str = _buffer.substr(from, to);
					str = [str stringByTrim: newline];
	...
	void editor_t::perform_replacements (std::multimap<OakTextRange *, NSString *>  replacements)
	{
		std::multimap<range_t, NSString *> tmp;
		riterate(pair, replacements)
		{
			// D(DBF_Editor, bug("replace %s with ‘%s’\n", NSString *(pair->first).c_str(), pair->second.c_str()););
			NSUInteger from = _buffer.convert(pair->first.min());
			NSUInteger to   = _buffer.convert(pair->first.max());
	...
	OakSelectionRanges * editor_t::insert_tab_with_indent (NSString *& buffer, OakSelectionRanges *  selections, snippet_controller_t& snippets)
	{
		...
		citerate(range, dissect_columnar(buffer, selections))
		{
			NSUInteger const from               = range->min().index;
			NSUInteger const firstLine          = buffer.convert(from).line;
			NSString * leftOfCaret  = buffer.substr(buffer.begin(firstLine), from);

			if(!text::is_blank(leftOfCaret.data(), leftOfCaret.data() + leftOfCaret.size()))
			{
				NSUInteger col = visual_distance(buffer, buffer.begin(firstLine), from);
				insertions.insert(std::make_pair(*range, buffer.indent().create(col)));
			}
			else
			{
				NSUInteger to = range->max().index;

				if(estimatedIndent == NULL_STR)
				{
					NSUInteger const lastLine           = buffer.convert(to).line;
					NSString * rightOfCaret = buffer.substr(to, buffer.end(lastLine));
	...
	OakSelectionRanges * editor_t::insert_newline_with_indent (NSString *& buffer, OakSelectionRanges *  selections, snippet_controller_t& snippets)
	{
		...
		citerate(range, dissect_columnar(buffer, selections))
		{
			...
			{
				NSUInteger n = firstLine;
				while(n > 0 && !fsm.is_seeded(indent::line(buffer, --n)))
					continue;
			}

			NSUInteger existingIndent = indent::leading_whitespace(rightOfCaret.data(), rightOfCaret.data() + rightOfCaret.size(), buffer.indent().tab_size());
			NSUInteger newIndent      = fsm.scan_line(rightOfCaret);
	...
	{
		ssize_t adjustment = 0;
		std::map<NSUInteger, NSString *> clips;
		citerate(range, dissect_columnar(_buffer, _selections).sorted())
		{
			range_t r = *range + adjustment;
			NSString * str = _buffer.substr(r.min().index, r.max().index);
			std::multimap<range_t, NSString *> replacements;
	...
	BOOL editor_t::handle_result (NSString * out, output::type placement, output_format::type format, output_caret::type outputCaret, OakTextRange * input_range, std::map<NSString *, NSString *> environment)
	{
		...
		NSUInteger caret = _selections.last().last.index;
		NSUInteger line = 0, column = 0, offset = 0;
		...
		switch(format)
		{
			...
			case output_format::text:
			{
				...
				{
					offset = utf8::find_safe_end(out.begin(), out.begin() + MIN(offset, out.size())) - out.begin();
					_selections = range_t(range.min().index + offset);
				}
				else if(outputCaret == output_caret::interpolate_by_line)
				{
					_selections = visual_advance(_buffer, _buffer.begin(MIN(line, _buffer.lines()-1)), column);
				}
			}
			break;
			...
			// {
			// 	std::vector<NSString *> completions;
	...
	void editor_t::delete_tab_trigger (NSString * str)
	{
		ranges_t ranges;
		iterate(range, _selections)
		{
			NSUInteger from = range->min().index;
			ranges.push_back(range_t(from - MIN(str.size(), from), range->max()));
		}
		_selections = apply(_buffer, ranges, _snippets, &transform::null);
	}

	OakScopeContext * editor_t::scope (NSString * scopeAttributes) const
	{
		return ng::scope(_buffer, _selections, scopeAttributes);
	}

	std::map<NSString *, NSString *> editor_t::variables (std::map<NSString *, NSString *> map, NSString * scopeAttributes) const
	{
		if(_document)
		...
		if(_selections.size() == 1)
		{
			...
			else
			{
				...
				{
					map.insert(std::make_pair("TM_SELECTED_TEXT", text::format("Error: Selection exceeds %s. Command should read selection from stdin.", OakStringFromFileSize(ARG_MAX-32).c_str())));
				}
			}
		}

		OakScopeContext *  s = scope(scopeAttributes);
