...
- (void)addAttribute: (NSString *)attrName
               value: (id)value
{
    [_attributes addObject: @[ @([_string length]), attrName, value]];
}
...
- (void)appendURL: (NSURL *)link
{
    if(link)
    {
        ...
        add(OakAttributedStringStyleUnderline);
    }
}

- (void)appendAttributedString: (OakAttributedString *)str
{
    add(OakAttributedStringStylePush);
    ...
    add(OakAttributedStringStylePop);    
}
