...
- (void)setAccessoryMenu: (NSMenu *)newAccessoryMenu
{
    if (newAccessoryMenu != accessoryMenu)
    {
        ...
        if (self.changeFontAndImageInMenu)
        {
            [self changeFontAndImageInMenu:self.accessoryMenu];
        }
...
- (void)setDelegate: (id<GCJumpBarDelegate>)delegate
{
    if (_delegate != delegate)
    {
        _delegate = delegate;
        
        _delegateRespondsToDidSelectAccessoryMenuItemAtIndex = [_delegate respondsToSelector: @selector(jumpBar:didSelectAccessoryMenuItemAtIndex:)];
        _delegateRespondsToDidSelectItemAtIndexPath = [_delegate respondsToSelector: @selector(jumpBar:didSelectItemAtIndexPath:)];
    }
}
...
- (void)setAccessoryMenuSelectedIndex: (NSUInteger)newAccessoryMenuSelectedIndex
{
    if (accessoryMenuSelectedIndex != newAccessoryMenuSelectedIndex)
    {
        ...
        if (_delegateRespondsToDidSelectAccessoryMenuItemAtIndex)
        {
            [_delegate jumpBar: self
didSelectAccessoryMenuItemAtIndex: self.accessoryMenuSelectedIndex];
        }
...
- (void) performLayoutIfNeededWithNewSize:(CGSize) size
{
    if (self.accessoryMenu != nil  || self.underIdealWidth)
    {
        [self performLayout];
...
- (void)removeUnusedLabels
{
    ...
    if (self.accessoryMenu == nil)
    {
        [[self viewWithTag:GCJumpBarAccessoryMenuLabelTag] removeFromSuperview];
    }
}
...
{
    if (label.tag == GCJumpBarAccessoryMenuLabelTag)
    {
        return self.accessoryMenu;
    }else
    {
...
- (void)jumpBarLabel:(GCJumpBarLabel *)label didReceivedClickOnItemAtIndexPath:(NSIndexPath *)indexPath
{
    if (label.tag == GCJumpBarAccessoryMenuLabelTag)
    {
        self.accessoryMenuSelectedIndex = [indexPath indexAtPosition:0];
    }else
    {
