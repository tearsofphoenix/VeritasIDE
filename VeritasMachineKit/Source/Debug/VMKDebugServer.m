...
- (void)handleClientConnection: (NSArray *)args
{
    ...
    @autoreleasepool
    {
        ...
        if (httpInitLine)
        {
            ...
            NSLog(@"%@ [VMKDebugServer] REQUEST HTTP INIT LINE: %@", stringFromCurrentTime(), httpInitLineString);
    ...
    if (!_done)
    {
        @autoreleasepool
        {
            if([self start: 9449])
            {
                ...
                VMKDebugCommandLaunchHandler *launchHandler = [VMKDebugCommandLaunchHandler handler: [HTMLData base64EncodedString]];
                [launchHandler setState: _state];
                
                [self registerHandler: launchHandler
