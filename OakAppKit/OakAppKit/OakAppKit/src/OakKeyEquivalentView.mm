...
- (void)flagsChanged:(NSEvent*)anEvent
{
	if(recording)
	{
		NSString * str = OakGlyphsForFlags([anEvent modifierFlags]);
		self.displayString = ([str isEqualToString:  @""] ? kRecordingPlaceholderString : str);
	}
}
...
- (void)keyDown:(NSEvent*)anEvent
{
	...
	else
	{
		...
		}else if([RecordingKeys containsObject: keyString])
        {
			self.recording = YES;
