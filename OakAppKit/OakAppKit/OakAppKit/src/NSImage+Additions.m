...
+ (NSImage*)imageNamed: (NSString*)aName
   inSameBundleAsClass: (Class)aClass
{
	if(!aName)
		return nil;

    NSImage *image = [self imageNamed: aName];
    if (image)
    {
        return image;
    }
    
    image = [[NSBundle bundleForClass: aClass] imageForResource: aName];
...
{
	[self drawAdjustedInRect: (NSRect){ aPoint, [self size] }
                    fromRect: srcRect
                   operation: op
                    fraction: delta];
}
